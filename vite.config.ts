...
async function searchDanggun(_req: IncomingMessage, res: ServerResponse) {
  try {
    ...
    const responses = await Promise.allSettled(SEARCH_REGIONS.map(async region => {
      const query = new URLSearchParams({ region_id:region.id, search:'pc', price:'100000__300000', only_on_sale:'true', uri, nonce:String(nonce), expires_at:String(expiresAt) });
    ...
    res.statusCode = 200; res.setHeader('Content-Type', 'application/json; charset=utf-8'); res.end(JSON.stringify({ listings, searchedRegions:SEARCH_REGIONS.length, failedRegions:responses.length - successful.length }));
  } catch (error) {
